---
title: "Android的UI显示原理之Surface的创建本文是Android视图层源码分析系列第二篇文章，通过上一篇文章深入剖 - 掘金"
source: "https://juejin.cn/post/6844903761292886030"
author:
published: 2019-01-15
created: 2025-04-03
description: "本文是Android视图层源码分析系列第二篇文章，通过上一篇文章深入剖析Window组成已经了解到WindowManager.addView(contentView)最终会交给WindowManagerService处理，WindowManagerService会创建这个con…"
tags:
  - "clippings"
---
![](https://p26-piu.byteimg.com/tos-cn-i-8jisjyls3a/ef2acfa660674ce49383c973812608ef~tplv-8jisjyls3a-image.image)

本文是 [Android视图层源码分析](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FSusionSuc%2FAdvancedAndroid%2Fblob%2Fmaster%2FAndroidFramework%25E6%25BA%2590%25E7%25A0%2581%25E5%2588%2586%25E6%259E%2590%2FAndroid%25E8%25A7%2586%25E5%259B%25BE%25E5%25B1%2582%25E6%25BA%2590%25E7%25A0%2581%25E5%2588%2586%25E6%259E%2590%2FREADME.md "https://github.com/SusionSuc/AdvancedAndroid/blob/master/AndroidFramework%E6%BA%90%E7%A0%81%E5%88%86%E6%9E%90/Android%E8%A7%86%E5%9B%BE%E5%B1%82%E6%BA%90%E7%A0%81%E5%88%86%E6%9E%90/README.md") 系列第二篇文章，通过上一篇文章 [深入剖析Window组成](https://juejin.cn/post/6844903753150103560 "https://juejin.cn/post/6844903753150103560") 已经了解到 `WindowManager.addView(contentView)` 最终会交给 `WindowManagerService` 处理， `WindowManagerService` 会创建这个 `contentView` 相关的UI载体 `Surface`,本文的目的就是理清 `Surface` 的创建过程，以及它与 `SurfaceFlinger` 的关系。

**文章比较长，但希望你可以坚持看完，相信会有一定收获的**

本文是基于 [Google Android Repo](https://link.juejin.cn/?target=https%3A%2F%2Fandroid.googlesource.com%2F "https://android.googlesource.com/") 中的较新的源码分析的。 `Android UI` 渲染这一块变化还是比较大的，逻辑可能和网上大部分文章有一些出入。
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/1/15/1684f466358d089b~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
在上一篇文章中我们知道， `ViewRootImpl` 管理着整个view tree。 对于 `ViewRootImpl.setView()` ，我们可以简单的把它当做一个 `UI渲染操作` 的入口，因此我们就从这个方法开始看:
...
```cpp
代码解读复制代码public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView) {
    ...
...
```cpp
代码解读复制代码public int addWindow(Session session, IWindow client...) {
    ...
...
```cpp
代码解读复制代码void windowAddedLocked(String packageName) {
    ...
...
```cpp
代码解读复制代码static jlong nativeCreate(JNIEnv* env, jclass clazz) {
...
```cpp
代码解读复制代码//这个方法在第一次使用SurfaceComposerClient的指针的时候会调用
...
```cpp
代码解读复制代码sp<ISurfaceComposerClient> SurfaceFlinger::createConnection() {
...
```cpp
代码解读复制代码class Client : public BnSurfaceComposerClient
{
public:
    ...
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/1/15/1684f46fa3007b7d~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
```cpp
代码解读复制代码public final Surface mSurface = new Surface();
```
...
```cpp
代码解读复制代码void scheduleTraversals() {
    ...
...
对于 `Choreographer` 本文不做详细的分析，可以去参考 [Android Choreographer 源码分析](https://link.juejin.cn/?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2F996bca12eb1d "https://www.jianshu.com/p/996bca12eb1d") 的分析来了解这个类。
...
```cpp
代码解读复制代码final class TraversalRunnable implements Runnable {
    @Override
    public void run() {
        doTraversal();
    }
...
```cpp
代码解读复制代码private void performTraversals() {
...
```cpp
代码解读复制代码private int relayoutWindow(WindowManager.LayoutParams params, ...) throws RemoteException {
    ...
...
```cpp
代码解读复制代码 //这里的outSurface其实就是ViewRootImpl中的那个Surface
...
```cpp
代码解读复制代码long mNativeObject; //成员指针变量，指向native创建的SurfaceControl
...
```cpp
代码解读复制代码static jlong nativeCreate(JNIEnv* env, ...) {
...
```cpp
代码解读复制代码//outSurface会指向新创建的SurfaceControl
...
```cpp
代码解读复制代码status_t Client::createSurface(...)
{
    ...
...
```cpp
代码解读复制代码status_t SurfaceFlinger::createLayer(const String8& name,const sp<Client>& client...)
{
...
```cpp
代码解读复制代码status_t SurfaceFlinger::createBufferLayer(const sp<Client>& client... sp<Layer>* outLayer)
{
    ...
...
```cpp
代码解读复制代码sp<IGraphicBufferProducer> BufferLayer::getProducer() const {
    return mProducer;
}
```
...
```cpp
代码解读复制代码void BufferLayer::onFirstRef() {
    ...
...
```cpp
代码解读复制代码void BufferQueue::createBufferQueue(sp<IGraphicBufferProducer>* outProducer,
    ...
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/1/15/1684f47389c36000~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
```cpp
代码解读复制代码private int createSurfaceControl(Surface outSurface, int result, WindowState win,WindowStateAnimator winAnimator) {
    ...
    surfaceController = winAnimator.createSurfaceLocked(win.mAttrs.type, win.mOwnerUid);
    ...
    surfaceController.getSurface(outSurface);
...
```cpp
代码解读复制代码//WindowSurfaceController.java
...
```cpp
代码解读复制代码static jlong nativeGetFromSurfaceControl(JNIEnv* env, jclass clazz, jlong surfaceControlNativeObj) {
...
```cpp
代码解读复制代码sp<Surface> SurfaceControl::getSurface() const
{
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/1/15/1684f477437887aa~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)

经过上面这个图，也可以理解 `SurfaceControl` 为什么叫 `SurfaceControl` 了。

本文到这里已经很长了，对于 **UI的渲染过程:即ViewRootImpl是如何渲染到Surface** 到下一篇文章再分析。

**欢迎关注我的 [Android进阶计划](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FSusionSuc%2FAdvancedAndroid "https://github.com/SusionSuc/AdvancedAndroid") 看更多干货**

**欢迎关注我的微信公众号:susion随心**

![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/1/15/1684f481f894ea02~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)

参考文章:

> [AndroidUI系列—浅谈图像渲染机制](https://link.juejin.cn/?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2F1998182670fb "https://www.jianshu.com/p/1998182670fb")

> [一篇文章看明白 Android 图形系统 Surface 与 SurfaceFlinger 之间的关系](https://link.juejin.cn/?target=https%3A%2F%2Fblog.csdn.net%2Ffreekiteyu%2Farticle%2Fdetails%2F79483406 "https://blog.csdn.net/freekiteyu/article/details/79483406")